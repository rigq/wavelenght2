...
const WINNING_SCORE = 10;      // puntos para ganar la partida
const MIN_PLAYERS = 4;         // mínimo para empezar
...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", ({ code, name }) => {
    ...
    if (player) {
      ...
      // Si la partida ya empezó, asignarle a un equipo para que pueda mirar
      if (room.started) {
    ...
    if (room.started && room.currentRound && getPsychicId(room) === player.id) {
  ...
  socket.on("start_game", () => {
    ...
    if (room.players.length < MIN_PLAYERS) {
      socket.emit("room_error", `Se necesitan al menos ${MIN_PLAYERS} jugadores.`);
      return;
    }
  ...
  socket.on("next_round", () => {
    ...
    // Si ya hay un ganador, no seguir (se usa play_again en su lugar)
